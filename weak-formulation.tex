...
%%----------------------------------------------------------------------------
\begin{frame}[t,allowframebreaks]{}\justifying

  \begin{defn}[Model equation for plane elasticity
    problems~\cite{Reddy2006}]\label{defn:model-equation}%
    \justifying
    ...
    \begin{description}
      \item[$f_{x}$:] the component of the body force vector (per unit
                      volume) along the $x$ direction.
      \item[$f_{y}$:] the component of the body force vector (per unit 
                      volume) along the $y$ direction.
      %\item[$\rho$:] The density of the material.
    \end{description}
  ...
  where $n_{x}$, $n_{y}$ denote the components (or the direction cosines)
  of the unit normal vector, $\mathbf{n}^{\top} = (n_{x}, n_{y})$, on the
  boundary of $\Omega$, $\Gamma = \partial{\Omega}$; $\Gamma_{\sigma}$ and
  $\Gamma_{u}$ are two disjoint pieces of $\Gamma$; $\hat{t}_{x}$ and
  $\hat{t}_{y}$ denote the components of the specified traction vector,
  $\mathbf{t}^{\top} = (t_{x}, t_{y})$; $\hat{u}$, and $\hat{v}$ are the
  components of the specified displacement vector $\mathbf{w}^{\top} =
  (u,v)$. 
...
\begin{frame}[t]{}\justifying
  % Put here the content of the 1st slide
