...
\newtheorem{defn}{Definició}%[thm]{Definici\'{o}}
\theoremstyle{remark}
...
\begin{frame}[t,allowframebreaks]{}\justifying
  ...
  \begin{defn}[Model equation for plane elasticity
    ...
    \justifying
    For the plane elasticity problems, the
    the normal stress in the $x$ direction, $\sigma_{xx} = \sigma_{xx}(x,y)$, the
    normal stress in the $y$ direction, $\sigma_{yy} = \sigma_{yy}(x,y)$, and the shear stress,
    $\sigma_{xy} = \sigma_{xy}(x,y)$, satisfy the BVP given by the two coupled PDE system:
  ...
  \right\}
    ...
    $(x,y)\in\Omega\subseteq\R^{2}$. Being, 
    \begin{description}
